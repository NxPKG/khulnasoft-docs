...
With static IPs, Khulnasoft commits to never changing the IP address of a client's domain resolved at the Khulnasoft global network. For example, `www.example.com` will always resolve and accept traffic sent to `198.51.100.10`. No other customer will be hosted on that IP.
...
Once you get your static IP from Khulnasoft, you can use it via API, just like [BYOIP](/byoip/). For the moment, there is still no UI available for this feature.

When creating a Spectrum application through the API, specify the static IPs that you have been provided. See, for instance, the API example below that creates an application routing traffic through Khulnasoft’s HTTP pipeline.
