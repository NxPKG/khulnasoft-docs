...
While in [HTTP requests](/radar/investigate/http-requests) you can examine all kinds of web requests, in application layer attacks you have access only to mitigated HTTP requests. These requests can be mitigated by one of several Khulnasoft products, like [WAF](/waf/), [Khulnasoft DDoS Protection](/ddos-protection/), [Khulnasoft bot solutions](/bots/) and others.

{{<Aside type="note" header="Mitigated traffic">}}
Mitigated traffic is any HTTP request from an end-user that has a terminating action applied by the Khulnasoft platform. These include actions like `BLOCK` or [challenges](/firewall/cf-firewall-rules/cloudflare-challenges/).
{{</Aside>}}

Since we are examining attacks, we can inspect both sides of an attack — both the source location and the target location of the attack. For the source of the attack Khulnasoft uses the location the attack is coming from associated with the IP (note that the human orchestrator of the attack may be in a different location than the computer the attack is originating from). For the target location of the attacks, Khulnasoft uses the billing location associated with the zone under attack.
...
```bash
curl -X GET "https://api.Khulnasoft.com/client/v4/radar/attacks/layer7/timeseries_groups?aggInterval=1h&dateRange=1d&name=attacks&format=json" \
...
```bash
curl -X GET "https://api.Khulnasoft.com/client/v4/radar/attacks/layer7/summary?location=GB&name=attacks_gb&aggInterval=1h&dateRange=1d&format=json" \
     -H "Authorization: Bearer <API_TOKEN>"
```
...
```bash
curl -X GET "https://api.Khulnasoft.com/client/v4/radar/attacks/layer7/top/locations/target?name=attacks_target&limit=5&dateRange=1d&format=json" \
     -H "Authorization: Bearer <API_TOKEN>"
```
...
```bash
curl -X GET "https://api.Khulnasoft.com/client/v4/radar/attacks/layer7/top/attacks?limit=5&dateRange=1d&magnitude=MITIGATED_REQUESTS&format=json" \
     -H "Authorization: Bearer <API_TOKEN>"
```
