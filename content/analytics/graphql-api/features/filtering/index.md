...
Add the `requestSource` filter for `eyeball` to return request, data transfer, and visit data about only the end users of your website. This will exclude actions taken by Khulnasoft products (for example, cache purge,  healthchecks, Workers subrequests) on your zone.  
