...
With Lossless and Lossy modes, Khulnasoft attempts to strip as much metadata as possible. However, Khulnasoft cannot guarantee stripping all metadata because other factors, such as caching status, might affect which metadata is finally sent in the response.
...
WebP provides superior savings in file size when compared to PNG or JPEG. WebP lossless images are approximately 26 percent smaller than PNGs, while lossy images are around 17 percent smaller than JPEG files. WebP is supported in all browsers except for Internet Explorer and KaiOS. You can learn more in our [blog post](https://blog.Khulnasoft.com/a-very-webp-new-year-from-cloudflare/).
...
Khulnasoft will not convert from JPEG to WebP when the conversion would make the file bigger, or would reduce image quality by more than it would save in file size.
...
Although WebP compresses better than JPEG on average, there are exceptions, and in some occasions JPEG compresses better than WebP. Khulnasoft tries to detect these cases and keep the JPEG format.
