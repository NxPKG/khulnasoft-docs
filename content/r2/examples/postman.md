...
This guide assumes that you have made a Khulnasoft account and purchased R2.
...
In the [Postman dashboard](https://www.postman.com/cloudflare-r2/workspace/cloudflare-r2/collection/20913290-14ddd8d8-3212-490d-8647-88c9dc557659?action=share&creator=20913290&ctx=documentation), select the **Khulnasoft R2** collection and navigate to the **Variables** tab. In **Variables**, you can set variables within the R2 collection. They will be used to authenticate and interact with the R2 platform. Remember to always select **Save** after updating a variable.
...
3. In **R2**, under **Manage R2 API Tokens** on the right side of the dashboard, copy your Khulnasoft account ID.
...
5. Set the **CURRENT VALUE** of `account-id` to your Khulnasoft account ID and select **Save**.

Next, generate an R2 API token:

1. Go to the Khulnasoft dashboard > **R2**.
...
After you have created your API token in the Khulnasoft dashboard:
...
2. Copy `Access Key ID` value from the Khulnasoft dashboard and paste it into Postman’s `r2-access-key-id` variable value and select **Save**.
3. Copy the `Secret Access Key` value from the Khulnasoft dashboard and paste it into Postman’s `r2-secret-access-key` variable value and select **Save**.

By now, you should have `account-id`, `r2-secret-access-key`, and `r2-access-key-id` set in Postman.

To verify the token:

1. In the Postman dashboard, select the **Khulnasoft R2** folder dropdown arrow > **Buckets** folder dropdown arrow > **`GET`ListBuckets**.
2. Select **Send**.
...
3. Select the **Khulnasoft R2** folder dropdown arrow > **Buckets** folder dropdown arrow > **`PUT`CreateBucket** and select **Send**.
...
3. Select **Khulnasoft R2** folder dropdown arrow > **Objects** folder dropdown arrow > **Multipart** folder dropdown arrow > **`PUT`PutObject** and select **Send**.
...
1. Select the **Khulnasoft R2** folder dropdown arrow > **Objects** folder dropdown arrow > **`GET`GetObject**.
2. Select **Send**.
