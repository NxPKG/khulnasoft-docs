...
source: https://support.Khulnasoft.com/hc/es-es/articles/200170786-C%C3%B3mo-restablecer-las-IP-originales-del-visitante
...
{{<Aside type="warning">}}
Khulnasoft ya no actualizará ni proveerá asistencia técnica para
...
Cuando el [tráfico de tu sitio web se enruta a través de la red de Khulnasoft](https://support.Khulnasoft.com/hc/articles/205177068), actuamos como un proxy inverso. Esto permite a Khulnasoft acelerar el tiempo de carga de página enrutando los paquetes de manera más eficiente y almacenando en caché los recursos estáticos (imágenes, JavaScript, CSS, etc.). Como resultado, al responder las solicitudes y registrarlas, tu servidor de origen devuelve una [dirección IP de Khulnasoft](https://www.Khulnasoft.com/ips/).

Por ejemplo, si instalas aplicaciones que dependen de la dirección IP entrante del visitante original, se registra por defecto una dirección IP de Khulnasoft. La dirección IP original del visitante aparece en un encabezado HTTP añadido denominado [_CF-Connecting-IP_](https://support.Khulnasoft.com/hc/articles/200170986). Siguiendo nuestras [instrucciones del servidor web](https://support.Khulnasoft.com/hc/articles/200170786#JUxJSMn3Ht5c5yq), puedes registrar la dirección IP original del visitante en tu servidor de origen. Si este encabezado HTTP no está disponible cuando las solicitudes llegan a tu servidor de origen, comprueba tu configuración de [Reglas de transformación](/rules/transform/) y [Transformaciones administradas](/rules/transform/managed-transforms/).

El siguiente diagrama muestra las diferentes formas en las que se gestionan las direcciones IP con y sin Khulnasoft.
...
Khulnasoft ya no actualiza ni admite _mod\_cloudflare._ Sin embargo, si utilizas un servidor web Apache con un sistema operativo como **Ubuntu Server 18.04** y **Debian 9 Stretch**, puedes utilizar _mod\_remoteip_ para registrar la dirección IP original de tu visitante.
...
4\. Define las direcciones proxy de confianza creando `/etc/apache2/conf-available/remoteip.conf` especificando el siguiente código y [direcciones IP de Khulnasoft](https://www.Khulnasoft.com/ips/):


{{<raw>}}<pre class="CodeBlock CodeBlock-with-rows CodeBlock-scrolls-horizontally CodeBlock-is-light-in-light-theme CodeBlock--language-txt" language="txt"><code><span class="CodeBlock--rows"><span class="CodeBlock--rows-content"><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">RemoteIPHeader CF-Connecting-IP</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">RemoteIPTrustedProxy 192.0.2.1 (dirección IP de ejemplo)</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">RemoteIPTrustedProxy 192.0.2.2 (dirección IP de ejemplo)</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">(repite este paso para todas las direcciones IP de Khulnasoft que aparecen en [https://www.Khulnasoft.com/ips/](https://www.Khulnasoft.com/ips/))</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">
</span></div></span></span></span></code></pre>{{</raw>}}
...
{{<raw>}}<pre class="CodeBlock CodeBlock-with-rows CodeBlock-scrolls-horizontally CodeBlock-is-light-in-light-theme CodeBlock--language-txt" language="txt"><code><span class="CodeBlock--rows"><span class="CodeBlock--rows-content"><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">set_real_ip_from 192.0.2.1 (dirección IP de ejemplo)</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">(repite este paso para todas las direcciones IP de Khulnasoft especificadas en [https://www.Khulnasoft.com/ips/](https://www.Khulnasoft.com/ips/))</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">
...
Esta lista de prefijos debe actualizarse periódicamente. Publicamos la lista completa en [Direcciones IP de Khulnasoft](https://www.Khulnasoft.com/ips).
...
Consulta también: [Khulnasoft y NGINX](https://danielmiessler.com/blog/getting-real-ip-addresses-using-cloudflare-nginx-and-varnish/).
...
Al usar Railgun (u otro software de proxy inverso, como Varnish) las solicitudes del usuario procederán del servidor Railgun y no de Khulnasoft. Puesto que las solicitudes no proceden directamente de Khulnasoft, los mod añadidos no restaurarán por defecto las direcciones IP de los visitantes.
...
2.  Al final, añade:`KhulnasoftRemoteIPTrustedProxy railgun_address`Por lo tanto, si tu servidor Railgun está ubicado en 127.0.0.1, tendrá la forma siguiente:`KhulnasoftRemoteIPTrustedProxy 127.0.0.1`
3.  Si tienes más de un servidor para añadir a la lista de proxies de confianza, puedes añadirlos al final:KhulnasoftRemoteIPTrustedProxy 127.0.0.1 127.0.0.2
...
</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">(repite este paso para todas las direcciones IP de Khulnasoft que aparecen en [https://www.Khulnasoft.com/ips/](https://www.Khulnasoft.com/ips/))</span></div></span><span class="CodeBlock--row"><span class="CodeBlock--row-indicator"></span><div class="CodeBlock--row-content"><span class="CodeBlock--token-plain">
</span></div></span></span></span></code></pre>{{</raw>}}
...
3.  Una vez activada, tus registros de acceso mostrarán las direcciones IP correctas, e incluso la variable `$_SERVER['REMOTE_ADDR']` de PHP contendrá la dirección IP real del cliente, en lugar de una dirección IP de Khulnasoft, que en sí misma resolverá la mayoría de los problemas que puedes encontrar al activar Khulnasoft en los sitios web activados por PHP (como las instalaciones de WordPress o vBulletin).
...
Consulta este tutorial de un tercero para ver cómo restaurar la dirección IP original del visitante con [Magento y Khulnasoft](https://tall-paul.co.uk/2012/03/02/magento-show-remote-ip-when-using-cloudflare/).

De igual modo, Khulnasoft no ha escrito [esta extensión  de Magento](https://marketplace.magento.com/), pero a algunos de nuestros clientes les ha resultado útil.
...
Para activar la compatibilidad correcta de las direcciones IP al ejecutar la instalación de Invision Power Board 3 en Khulnasoft, sigue estas instrucciones:
...
Si utilizas un servidor Apache, recomendamos instalar [mod\_cloudflare](https://support.Khulnasoft.com/hc/articles/200170786#C5XWe97z77b3XZV) para restaurar la dirección IP del visitante en tus registros.
...
Como alternativa, puedes instalar el [complemento de gestión de Khulnasoft](https://mods.mybb.com/view/antoligy-mybb-cloudflare-management-plugin) disponible para MyBB 1.6.
...
Un miembro del equipo de Vanilla ha escrito un [complemento de Khulnasoft para Vanilla](https://open.vanillaforums.com/addon/cloudflaresupport-plugin) para restaurar la dirección IP original de los visitantes a los archivos de registro para los sitios autoalojados.
...
Un usuario de Xenforo ha creado un [complemento para Khulnasoft](https://xenforo.com/community/resources/solidmean-cloudflare-detect.1595/).
...
Un tercero ha creado un [módulo para Khulnasoft y PunBB](http://punbb.informer.com/forums/post/147539/#p147539) que restaurará la dirección IP original del visitante.

Como este complemento es de un tercero, no podemos brindar asistencia técnica para los problemas relacionados con el complemento.

1.  Descarga `cherokee-admin` en tu servidor.
2.  Ve a la **interfaz de administración de Cherokee** en tu navegador web.
3.  Selecciona el **servidor virtual** para el dominio al que presta servicio Khulnasoft.
4.  En la pestaña _Logging_ (Registro) para el **servidor virtual** seleccionado, activa Accept Forwarded IPs (Aceptar IP enviadas).
5.  En el recuadro _Accept from Hosts_ (Aceptar de los hosts), especifica las [direcciones IP de Khulnasoft](https://www.Khulnasoft.com/ips/).
...
Un desarrollador externo ha creado una [extensión de Khulnasoft para TYPO3](https://extensions.typo3.org/extension/cloudflare/) que restablecerá la dirección IP original de los visitantes en tus registros. La extensión también te permitirá limpiar tu caché de Khulnasoft.
...
Debido a este proxy Nginx, en realidad necesitas las instrucciones para configurar Nginx para restituir la verdadera dirección IP de los visitantes.[Mod\_remoteip](https://support.Khulnasoft.com/hc/articles/200170786#C5XWe97z77b3XZV) para Apache no es necesario a menos que desactives el servidor Nginx para algunas solicitudes. Añadir [mod\_remoteip](https://support.Khulnasoft.com/hc/articles/200170786#C5XWe97z77b3XZV) a Apache no estará en conflicto con la configuración del servidor Nginx.
...
1.  Crea un archivo de texto CF`_ips.lst` que contenga todos los rangos de direcciones IP de https://www.Khulnasoft.com/en-gb/ips/.
