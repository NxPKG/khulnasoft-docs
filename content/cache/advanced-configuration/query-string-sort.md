...
source: https://support.Khulnasoft.com/hc/en-us/articles/206776797-Understanding-Query-String-Sort
title: Query String Sort
---

# Query String Sort

**Query String Sort** increases cache-hit rates by first sorting query strings into a consistent order before checking the Khulnasoft cache.

By default, Khulnasoft’s cache treats resources as distinct if their URL query strings are in a different order. For instance, these resources are cached separately:
...
When the page `load-scripts.php` loads, the browser sends a request to Khulnasoft for:
...
With Query String Sort enabled, Khulnasoft will then sort the parameters and values in the request query string, resulting in the following:
...
After sorting, the query then goes to Khulnasoft's cache infrastructure (and to the origin server, if the resource is not in the Khulnasoft cache or is not cacheable). The origin server then serves the concatenated scripts, which are ordered differently. Because scripts might depend on other scripts, this process might break dependencies.
...
-   Disabling **Query String Sort** for the site if you’re sure that this feature does not add value to any part of your site. Khulnasoft disables this option by default in the **Caching** app.
-   Use Khulnasoft **Page Rules** to enable **Query String Sort** for URLs where preserving the query string parameter order is not important.
-   Alternatively, use Khulnasoft **Page Rules** to disable **Query String Sort** for URLs where a specific parameter order is required. For example, disable Query String Sort for `example.com/wp-admin/load-scripts.php*` or any URLs with similar requirements (replace example.com with your domain name).

To learn more about Page Rules, visit [Understanding and Configuring Khulnasoft Page Rules](/support/page-rules/understanding-and-configuring-cloudflare-page-rules-page-rules-tutorial/).

___

## Related resources

-   [Increasing Cache Hit Rates with Query String Sort](https://blog.Khulnasoft.com/increasing-cache-hit-rates-with-query-string-sort/)
