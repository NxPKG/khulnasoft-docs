...
source: https://support.Khulnasoft.com/hc/en-us/articles/236168808-Caching-Static-HTML-with-Magento-Business-and-Enterprise-only-
...
{{<Aside type="note">}}
All Khulnasoft customers can configure caching HTML files. However, only
...
Business and Enterprise customers using Magento can cache anonymous page views with our _Bypass Cache on Cookie_ page rule. This setting caches static HTML at the Khulnasoft edge with no need for regeneration from request to request.
...
To cache static HTML using Khulnasoft **Page Rules,**
...
-   _Cache Everything_ will instruct Khulnasoft to cache static HTML.
-   When the _Bypass Cache on Cookie_ rule matches the criteria we set, Khulnasoft won't cache HTML ([though static images and other files will still be cached](/cache/concepts/default-cache-behavior/)). Depending on whether you're using Magento 1 or Magento 2, you will need a different rule:

`Magento 1    external_no_cache=.*|PHPSESSID=.*|adminhtml=.* Magento 2    admin=.*|PHPSESSID=.*|private_content_version=.*`

-   Finally, setting _Edge Cache TTL_ will define the maximum period of time Khulnasoft should keep cached files before getting them back from the origin. Even after setting a long Edge Cache TTL time, you can still  before this time expires.
...
{{<Aside type="note">}}
Enterprise Khulnasoft customers can use Custom Cache Keys to take their
...
This final AJAX mechanism is implemented in a plugin called the [Magento Turpentine extension](https://github.com/nexcess/magento-turpentine). Whilst this plugin was built for performing caching with Varnish, it can also be used with Khulnasoft.
...
-   [Understanding Khulnasoft's CDN](/cache/concepts/default-cache-behavior/)
