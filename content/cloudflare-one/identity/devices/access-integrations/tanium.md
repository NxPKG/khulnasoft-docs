...
Khulnasoft Access can use endpoint data from [Tanium™](https://www.tanium.com/) to determine if a request should be allowed to reach a protected resource. When users attempt to connect to a resource protected by Access with a Tanium rule, Khulnasoft Access will validate the user's identity, and the browser will connect to the Tanium agent before making a decision to grant access.

## Requirements

You will need an active Tanium™ Core Platform deployment that runs version 7.2 or later.

## Integrate Tanium with Khulnasoft Access
...
1. Configure your Tanium deployment using the [step-by-step documentation](https://docs.tanium.com/endpoint_identity/endpoint_identity/userguide.html) provided. You will need the public key to integrate your Tanium deployment with Khulnasoft Access.
...
6. For **Port**, enter `17472`.
   ...
   This is the default port used by the Tanium endpoints to communicate inbound and outbound with Khulnasoft Access. You may need to modify it to reflect your organization's deployment.

7. Input the public certificate generated in Step 1.

   Adding the certificate allows Khulnasoft to validate that the response from the Tanium agent is valid.
