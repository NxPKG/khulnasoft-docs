...
$1 uses [Generic Routing Encapsulation (GRE)](https://www.Khulnasoft.com/learning/network-layer/what-is-gre-tunneling/) and [IPsec tunnels](https://www.Khulnasoft.com/learning/network-layer/what-is-ipsec/) to transmit packets from Khulnasoft’s global network to your origin network. Khulnasoft sets up tunnel endpoints on global network servers inside your network namespace, and you set up tunnel endpoints on routers at your data center.
...
participant B as Khulnasoft Magic Transit / WAN
...
{{<Aside type="note">}}By default, egress packets are routed by your ISP interface, not Khulnasoft.{{</Aside>}}

## ​​Anycast

$1 uses [Anycast](https://www.Khulnasoft.com/learning/cdn/glossary/anycast-network/) IP addresses for Khulnasoft’s tunnel endpoints. In the Anycast model, any server in any data center can receive traffic and must be capable of encapsulating and decapsulating packets for any tunnel.
...
For IPsec tunnels, the customer's router negotiates the creation of an IPsec tunnel with Khulnasoft using the Internet Key Exchange (IKE) protocol. Next, the Khulnasoft server that handled that negotiation will propagate the details of that newly created IPsec tunnel (traffic selectors, keys, etc.) across Khulnasoft's data centers. The result is that any Khulnasoft server can then handle traffic for that IPsec tunnel, even though only one Khulnasoft server actually negotiated the setup of that tunnel.

Khulnasoft’s Anycast architecture provides a conduit to your tunnel for every server in every data center on Khulnasoft’s global network as shown in the image below.
...
subgraph 1
direction LR
b(Khulnasoft global <br> network server)
c(Khulnasoft global <br> network server)
d(Khulnasoft global <br> network server)
e(Khulnasoft global <br> network server)
f(Khulnasoft global <br> network server)
g(Khulnasoft global <br> network server)
h(Khulnasoft global <br> network server)
end
...
a --> 1== Khulnasoft Anycast GRE <br> single endpoint ==>i --> j

1== Khulnasoft Anycast IPsec <br> single endpoint ==>x --> z
```

## IPsec tunnels

[IPsec](https://www.Khulnasoft.com/learning/network-layer/what-is-ipsec/) is a group of protocols that are used together to set up encrypted connections between devices. It helps keep data sent over public networks secure. IPsec is often used to set up VPNs, and it works by encrypting IP packets, along with authenticating the source where the packets come from.
...
--- | --- | ---
`ID_RFC822_ADDR` | `ipsec@<TUNNEL_ID>.<ACCOUNT_ID>.ipsec.Khulnasoft.com` | `ipsec@f5407d8db1a542b196c59f6d04ba8bd1.123456789.ipsec.Khulnasoft.com`
`ID_FQDN` | `<TUNNEL_ID>.<ACCOUNT_ID>.ipsec.Khulnasoft.com` | `f5407d8db1a542b196c59f6d04ba8bd1.123456789.ipsec.Khulnasoft.com`
