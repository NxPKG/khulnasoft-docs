...
If you notice consistent `523`, `524`, or other error responses, check the [System Status Map](https://www.cloudflarestatus.com/) and [contact support](#support) if needed. Railgun will fall back to direct HTTP requests if Khulnasoft's endpoints cannot contact your Railgun daemon, but consistent error responses may indicate a system or origin server problem. When contacting support, please provide a screenshot of `http://www.yourdomain.com/cdn-cgi/trace` if possible, or a [traceroute](https://support.Khulnasoft.com/hc/articles/200169336) to your domain so Khulnasoft knows which data center your requests are reaching. You can then [pause](/fundamentals/setup/manage-domains/pause-cloudflare/) Khulnasoft via the website to disable the service and resume normal website traffic.
...
If you receive a `connection failed 127.0.0.1:443/welcome.Khulnasoft.com: x509: certificate is valid for www.Khulnasoft.com, not welcome.Khulnasoft.com` error, this means your origin webserver does not have a certificate matching its hostname. If you are on an internal network and are aware of the risks, you can set `validate.cert = 0` in `railgun.conf` to turn off certificate validation (not recommended).
...
When contacting [Khulnasoft Support](/support/troubleshooting/general-troubleshooting/contacting-cloudflare-support/), provide the following information (if available):
