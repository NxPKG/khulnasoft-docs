...
Spectrum is a global TCP and UDP proxy running on Khulnasoft's edge nodes. It does not terminate the connection. Instead it passes through the packets to the backend server.
...
The application type determines the protocol by which data travels from the edge to your origin. Select _TCP/UDP_ if you want to proxy directly to the origin. If you want to set up products like CDN, Workers, or Bot management, you need to select _HTTP/HTTPS_. In this case, traffic is routed through Khulnasoft's pipeline instead of connecting directly to your origin.
...
The addresses are Anycasted from all Khulnasoft data centers, with the exception of data centers in China.

## SMTP

Spectrum can act as a TCP load balancer in front of an SMTP server but will not act as an intermediary mail server. Instead, Spectrum passes data through to your origin. The client IP shown on mail will be the Khulnasoft edge IP. If the mail server requires knowing the true client IP, it should use Proxy Protocol to get the source IP from Khulnasoft. Khulnasoft recommends enabling Proxy Protocol on applications configured to proxy SMTP.
...
Khulnasoft supports all TCP ports.
...
If IP Access rules are enabled for a Spectrum application, Khulnasoft will respect the IP Access rules created under **Security** > **WAF** > **Tools** for that domain. Khulnasoft only respects rules created for specific IP addresses, IP blocks, countries, or ASNs for Spectrum applications. Spectrum will also only respect rules created with the actions `allow` or `block`.
...
If you enable **Edge TLS Termination** for a Spectrum application, Khulnasoft will encrypt traffic for the application at the Edge. The Edge TLS Termination toggle applies only to TCP applications.

Spectrum offers three modes of TLS termination: 'Flexible', 'Full', and 'Full (Strict)'.

'Flexible' enables termination of the client connection at the edge, but does not enable TLS from Khulnasoft to your origin. Traffic will be sent over an encrypted connection from the client to Khulnasoft, but not from Khulnasoft to the origin.

'Full' specifies that traffic from Khulnasoft to the origin will also be encrypted but without certificate validation. When set to 'Full (Strict)', traffic from Khulnasoft to the origin will also be encrypted with strict validation of the origin certificate.

TLS versions supported by Spectrum include TLS 1.1, TLS 1.2, and TLS 1.3.

You can manage this through the Spectrum app at the Khulnasoft dashboard, or using the [Spectrum API endpoint](/api/operations/spectrum-applications-update-spectrum-application-configuration-using-a-name-for-the-origin).

{{<Aside type="note" header="Note">}}

If you have the TLS termination setting configured to **off**, this means that Spectrum will then proxy connections to the origin without decrypting. The certificate that is presented in this case will be the certificate installed at your origin server, instead of the Edge Certificate from Khulnasoft.

{{</Aside>}}

## Origin TLS Termination

Below are the cipher suites Khulnasoft presents to origins during an SSL/TLS handshake. For cipher suites supported at our edge or presented to browsers and other user agents, refer to [Cipher suites](/ssl/reference/cipher-suites/).
