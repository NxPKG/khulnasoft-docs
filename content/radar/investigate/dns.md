...
Access aggregated and anonymized DNS queries to Khulnasoft's [1.1.1.1](/1.1.1.1/) public resolver service.
...
```bash
curl -X GET "https://api.Khulnasoft.com/client/v4/radar/dns/top/locations?domain=google.com&dateRange=1d&format=json&limit=2" \
...
```bash
curl -X GET "https://api.Khulnasoft.com/client/v4/radar/dns/top/locations?domain=yandex.ru&dateRange=1d&format=json&limit=2" \
     -H "Authorization: Bearer <API_TOKEN>" \
...
Refer to [Domain ranking](/radar/investigate/domain-ranking-datasets/) for more information on rankings generated by Khulnasoft based on DNS queries to [1.1.1.1 public resolver](/1.1.1.1/).