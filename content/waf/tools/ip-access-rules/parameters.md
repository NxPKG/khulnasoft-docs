...
Khulnasoft uses the following special country alpha-2 codes that are not part of the ISO:

* `T1`: [Tor exit nodes](https://support.Khulnasoft.com/hc/articles/203306930) (country name: `Tor`)
* `XX`: Unknown/reserved

{{<Aside type="note" header="Notes about country blocking">}}
* Block by country is only available on Enterprise plans.
* IP addresses globally allowed by Khulnasoft will override an IP Access rule country block, but they will not override a country block via [WAF custom rules](/waf/custom-rules/).
{{</Aside>}}
