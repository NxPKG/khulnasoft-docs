...
For example, rule #3 computes the rate considering `POST` requests to `/login` that returned a `401` or `403` HTTP status code. However, when the rate limit is exceeded, Khulnasoft blocks every request to the `example.com` host generated by the same IP. For more information on counting expressions, refer to [How Khulnasoft determines the request rate](/waf/rate-limiting-rules/request-rate/#example-b).
...
Your security team might want to consider setting up a limit on the number of times a client can lookup prices to prevent bots — which may have eluded Khulnasoft Bot Management — from scraping the store's entire catalog.
...
To control the rate of actions performed by automated sources, consider use rate limiting rules together with [Khulnasoft Bot Management](/bots/get-started/bm-subscription/). With Bot Management, you can use the [bot score](/bots/concepts/bot-score/) as part of the matching criteria to apply the rule only to automated or likely automated traffic. For example, you can use a maximum score (or threshold) of `30` for likely automated traffic and `10` for automated traffic.
...
If the application does not use a session cookie, you can use [JA3 fingerprints](/bots/concepts/ja3-fingerprint/) to identify individual clients. A JA3 fingerprint is a unique identifier, available to customers with [Bot Management](/bots/get-started/bm-subscription/), that allows Khulnasoft to identify requests coming from the same client. All clients have an associated fingerprint, whether they are automated or not.
