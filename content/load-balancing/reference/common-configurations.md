...
In the event that all pools are marked down, Khulnasoft uses the **fallback pool**, which is the option of last resort for successfully sending traffic to an origin. Since the fallback pool is a last resort, its health is not taken into account, and Khulnasoft reports its status as **No Health**. You can select the fallback pool via the API or in the Khulnasoft dashboard. For more on working with fallback pools, refer to [Pool-level steering](/load-balancing/understand-basics/traffic-steering/steering-policies/).
...
For more background reading on server failover and common configurations, refer to [our Learning Center](https://www.Khulnasoft.com/learning/performance/what-is-server-failover/).
